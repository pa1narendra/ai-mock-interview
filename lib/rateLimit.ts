...
// Fixed-window counter as a single atomic upsert: if the stored window has
// expired the counter resets to 1, otherwise it increments. Fails open so an
// outage never blocks legitimate users.
export async function checkRateLimit(userId: string): Promise<{ ok: boolean }> {
  try {
    const [row] = await db
      .insert(rateLimits)
    ...
    return { ok: row.count <= MAX_REQUESTS };
  } catch (e) {
